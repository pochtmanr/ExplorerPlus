...
const llamaPath = path.resolve(__dirname, '../llama.cpp/build/bin/main');
const modelPath = path.resolve(process.env.HOME, 'models/llama-2-7b.Q4_K_M.gguf');

// Verify paths exist
console.log('Checking if llama.cpp executable exists:', fs.existsSync(llamaPath));
console.log('Checking if model file exists:', fs.existsSync(modelPath));
...
app.post('/generate-itinerary', async (req, res) => {
  try {
    ...
    console.log('Executing llama.cpp with prompt...');
    
    // Call the local Llama model
    exec(`"${llamaPath}" -m "${modelPath}" -p "${prompt}" --temp 0.7 -n 2048`, 
      ...
      (error, stdout, stderr) => {
        ...
        console.log('LLM output received, parsing...');
...
function generateMockItinerary(location, days, res) {
...
app.listen(port, () => {
  ...
  console.log(`Using llama.cpp at: ${llamaPath}`);
